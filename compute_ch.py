...
def stdev(df, keys, cols, periods):
    for col in cols:
        for p in periods:
            df[f'{col}:標準差{p}'] = df.groupby(keys)[col].apply(lambda df: df.rolling(window=p).std())
    return df
...
df['Upper_Band'] = df.groupby(keys).apply(lambda df: df['Avg_Band'] + df['price:標準差20']*2).reset_index(drop=True)
df['Lower_Band'] = df.groupby(keys).apply(lambda df: df['Avg_Band'] - df['price:標準差20']*2).reset_index(drop=True)
df['(price-Avg_Band)/price:標準差20'] = df.groupby(keys).apply(lambda df: (df['price']-df['Avg_Band'])/df['price:標準差20']).reset_index(drop=True)

# b band adj
df['Avg_Band:調整'] = df.groupby(keys)['price:調整'].apply(lambda df: df.rolling(window=20).mean())
df['Upper_Band:調整'] = df.groupby(keys).apply(lambda df: df['Avg_Band:調整'] + df['price:調整:標準差20']*2).reset_index(drop=True)
df['Lower_Band:調整'] = df.groupby(keys).apply(lambda df: df['Avg_Band:調整'] - df['price:調整:標準差20']*2).reset_index(drop=True)
df['(price:調整-Avg_Band:調整)/price:調整:標準差20'] = df.groupby(keys).apply(lambda df: (df['price:調整']-df['Avg_Band:調整'])/df['price:調整:標準差20']).reset_index(drop=True)
...
df['RSI'] = df.groupby(keys).apply(lambda df: df['ch_u'].ewm(alpha=1/14).mean() / (df['ch_u'].ewm(alpha=1/14).mean() + df['ch_d'].ewm(alpha=1/14).mean())*100).reset_index(drop=True)
...
df = df.drop(['開盤價', '最高價', '最低價', '收盤價', 'price:標準差20', 'price:調整:標準差20'], axis=1)
list(df)
