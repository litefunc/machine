...
def stdev(df, keys, cols, periods):
    for col in cols:
        for p in periods:
            df[f'{col}:stdev{p}'] = df.groupby(keys)[col].apply(lambda df: df.rolling(window=p).std())
    return df

df = stdev(df, keys, ['price', 'price:調整'], [20])

# macd, osc
df['EMA12'] = df.groupby(keys)['price'].apply(lambda x: x.ewm(alpha=2/13).mean())
df['EMA26'] = df.groupby(keys)['price'].apply(lambda x: x.ewm(alpha=2/27).mean())
...
# b band
df['Avg_Band'] = df.groupby(keys)['price'].apply(lambda df: df.rolling(window=20).mean())
df['Upper_Band'] = df.groupby(keys).apply(lambda df: df['Avg_Band'] + df['price:stdev20']*2).reset_index(drop=True)
df['Lower_Band'] = df.groupby(keys).apply(lambda df: df['Avg_Band'] - df['price:stdev20']*2).reset_index(drop=True)
df['(price-Avg_Band)/stdev20'] = df.groupby(keys).apply(lambda df: (df['price']-df['Avg_Band'])/df['price:stdev20']).reset_index(drop=True)

# b band adj
df['Avg_Band:調整'] = df.groupby(keys)['price:調整'].apply(lambda df: df.rolling(window=20).mean())
df['Upper_Band:調整'] = df.groupby(keys).apply(lambda df: df['Avg_Band:調整'] + df['price:調整:stdev20']*2).reset_index(drop=True)
df['Lower_Band:調整'] = df.groupby(keys).apply(lambda df: df['Avg_Band:調整'] - df['price:調整:stdev20']*2).reset_index(drop=True)
df['(price:調整-Avg_Band:調整)/stdev20:調整'] = df.groupby(keys).apply(lambda df: (df['price:調整']-df['Avg_Band:調整'])/df['price:調整:stdev20']).reset_index(drop=True)
...
def Return(df, keys, cols, periods):
    for days in periods:
        ...
        df[__cols] = df.groupby(keys)[cols].apply(lambda df: (df.shift(-days)/df)**n-1)
    return df
...
def lnReturn(df, keys, cols, periods):
    for days in periods:
        ...
        df[__cols] = df.groupby(keys)[cols].apply(lambda df: np.log(df.shift(-days)/df)*n)
    return df 
...
def Normalize(df, keys, cols):
    ...
    df[__cols] = df.groupby(keys)[cols].apply(lambda df: (df-df.mean())/df.std())
    return df
...
def ma(df, keys, cols, period):
    for n in period:
        ...
        df[__cols] = df.groupby(keys)[cols].apply(lambda df: df.rolling(window=n).mean())
    return df
...
df['RSI:收盤價'] = df.groupby(keys).apply(lambda df: df['ch_u'].ewm(alpha=1/14).mean() / (df['ch_u'].ewm(alpha=1/14).mean() + df['ch_d'].ewm(alpha=1/14).mean())*100).reset_index(drop=True)
df = df.drop(['ch', 'ch_u', 'ch_d'], axis=1)

# kdj
df['max9'] = df.groupby(keys)['最高價'].apply(lambda df: df.rolling(window=9).max())
df['min9'] = df.groupby(keys)['最低價'].apply(lambda df: df.rolling(window=9).min())
df['rsv'] = df.groupby(keys).apply(lambda df: (df['收盤價']-df['min9'])/(df['max9']-df['min9'])).reset_index(drop=True)
...
list(df)
df = df.drop(['權值+息值','開盤價', '最高價', '最低價', '收盤價', 'price:stdev20', 'price:調整:stdev20'], axis=1)
